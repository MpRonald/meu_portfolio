...
def create_app() -> Flask:
    ...
    def weather():
        ...
        try:
            ...
            # ✅ ALTERAÇÃO pedida: o template usa chart.prob
            prob = [None] * len(dates)
    ...
    # ============================================================
    # ==== STUBS (para o index.html não rebentar) ===============
    # ==== depois substituímos 1 feature de cada vez ============
    # ============================================================
    ...
    def ecom_dashboard():
        return safe_render(
            "ecom.html",
            erro="(Em construção) A migrar para o novo app.",
            kpis=None,
    ...
    def quotes():
        return safe_render("quotes.html", error="(Em construção) A migrar para o novo app.", form={}, presets=[], notice=None,
                           chart_data=None, metrics=None, forecast=None, alg_label=None, stock_plot_div=None)

    @app.route("/fx", methods=["GET", "POST"])
    def fx():
        return safe_render("fx.html", erro="(Em construção) A migrar para o novo app.", resultado=None, pairs={})

    @app.route("/ml/heart", methods=["GET", "POST"])
    def ml_heart():
        return safe_render("ml_heart.html", error="(Em construção) A migrar para o novo app.", result=None, prob=None,
                           defaults={}, stats=None, features=[], heart_info={})

    @app.route("/nlp/supervised", methods=["GET", "POST"])
    def nlp_supervised():
        return safe_render("nlp_supervised.html", error="(Em construção) A migrar para o novo app.",
                           result=None, score=None, meta={}, engine="(Em construção)", stats=None, defaults={"text": ""})

    @app.route("/loan", methods=["GET"])
    def loan_form():
        return safe_render("loan.html", opcoes={}, erro="(Em construção) A migrar para o novo app.")
    ...
    def churn_xai_dashboard():
        return safe_render("churn_xai.html", erro="(Em construção) A migrar para o novo app.",
                           result=None, prob_pct=None, defaults={})
